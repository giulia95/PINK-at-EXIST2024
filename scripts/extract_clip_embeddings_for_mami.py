...
if __name__ == "__main__":
    ...
    parser.add_argument('--split-path', required=True, type=str, help='Path to the CSV representing the dataset split from which the embeddings will be extracted')
    ...
    for idx, sample in tqdm(split.iterrows()):
        sample_id = sample['file_name'].split('.')[0]

        # -- extracting embeddings
        image = Image.open( os.path.join(split_dir, 'test' if 'test' in args.split_path.lower() else "TRAINING", sample['file_name']) )
        text = sample['Text Transcription'].strip()
