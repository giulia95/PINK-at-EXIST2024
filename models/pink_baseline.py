...
class PinkBaseline(torch.nn.Module):

  def __init__(self, args):
    super(PinkBaseline, self).__init__()
    ...
    self.classifier = torch.torch.nn.Linear(
        args.model_conf['latent_dim'],
        args.model_conf['num_classes'],
        bias=False,
    )

    # 5. Computing Loss Function
    if args.training_settings['loss_criterion'] == 'CrossEntropy':
  ...
  def forward(self, batch):
    ...
    for modality_id in self.modality_map.keys():
      ...
      data = data + self.language_emb(language_ids)
